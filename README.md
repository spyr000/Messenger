...
- **От себя**
    - написан Dockerfile, docker-compose.yml, все переменные среды вынесены в .env файл, образ залит на dockerhub;
    - использование Liquibase;
    - добавлена возможность выбирать формат возвращаемых данных при помощи передачи параметра format в url запроса
...
### Если работа приложения во время выполнения удаления пользователя была приостановлена то при перезапуске сервера удаление возобновится
### (При запуске приложения все пользователи на которых ссылаются в таблице users_to_delete удаляются планировщиком в назначенное время удаления)
